...
class Agent():
    ...
    def update_position(self,agents,obstacles,exits,dt,size_scene):
        ...
        def time_calculus_points(X,X2,U,r):
            ...
            C = dot(X-X2,X-X2)-(r)**2
            DELTA2 = B**2-4*A*C
            if round(DELTA2,3)>0 and (-B-DELTA2**0.5)/(2*A)>=0 :
        ...
        def update_time_move_agents(time_move,agents,dt):
            ...
            for agent in agents:
                if agent!=self:
                    ...
                    r1 = self.size
                    r2 = agent.size
                    if norm(X-X2)<r1+r2+dt*norm(U):  #S'il y a chauvechement mais je ne comprends pas l'interet du troisième terme
                        A = dot(U,U)
                        B = 2*(dot(X,U)-dot(X2,U)) 
                        ...
                        if DELTA2>0:
                            ...
                            if round(sol,3)>=0 and sol<=time_move:
                                ...
                                #alpha = 2000
                                #beta = 0.08
                                #interactive_force = alpha*exp(-(distance_agents-self.size-agent.size)/beta)*direction
                                #L+=[interactive_force]
                                #A = [i for i in range(len(agents)+1)]
                                #x = np.array(A)
                                #y = np.array(L)
                                #plt.plot(x, y)
                                #plt.show()
        ...
        time_move = update_time_move_scenery(time_move, exits, dt,'exit')
    ...
    def get_color_agent(self):
        return min(exp(self.pressure+sum(self.pressure_interaction.values())-1),1)